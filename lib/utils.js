...
function parse(bufer, currentCommand) {
  ...
  if (MESSAGE_TYPE === 2 || MESSAGE_TYPE === 7) {
    ...
    result.info.statuses = resultStates.map(({ name, description, denomination, type, negative }) => {
  ...
  if (MESSAGE_TYPE === 7) {
    EXPANDED_DATA = DATA.slice(6, DATA.length)
    // if (currentCommand === 'QUERY_EXPANDED_NOTE_SPECIFICATION') {
    ...
    result.info.note = {
      denomination: result.info.expanded.baseValue * Math.pow(10, result.info.expanded.exponent)
    }
  }
